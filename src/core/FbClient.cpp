// FbClient.cpp — Implementation of FbClient (Phase 14.2.1).
//
// Replaces CFbDll for fbclient loading. Loads the native library,
// initializes the Firebird OO API, and creates an fbcpp::Client.
...
bool FbClient::LoadDll (const char * client, const char * clientDef)
{
#ifdef _WINDOWS
...
#define __ENTRYPOINT_OOAPI(X) _##X = (X*)dlsym(_Handle, "fb_"#X)
	...
	// Phase 14.2.1: Create fb-cpp Client from the loaded IMaster.
	// This gives us access to the modern C++ wrapper API.
	client_ = std::make_unique<fbcpp::Client>(_master);

#undef __ENTRYPOINT
#undef __ENTRYPOINT_OOAPI

	return true;
}

void FbClient::Release(void)
{
	// Release fb-cpp Client first (before tearing down master/provider)
	client_.reset();
