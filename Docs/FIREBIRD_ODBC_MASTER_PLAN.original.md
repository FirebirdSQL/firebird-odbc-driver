...
| **14.4.3** | **Migrate result fetching to fbcpp** — Output `Sqlda::buffer` NOT remapped to fbcpp::outMessage (IRD records cache `dataPtr` at `defFromMetaDataOut()` time; remapping desynchronizes when static cursors remap sqlvar to internal row blocks). RowSet migration deferred (requires fbcpp to own the cursor, conflicts with ODBC separate execute/fetch model). Current 64-row prefetch equivalent. See Phase 14.4.7 for detailed analysis. | Hard | ⏸️ Deferred |
...
| **14.4.7.2** | **Eliminate Sqlda output buffer** — Deferred. IRD records cache `dataPtr` at `defFromMetaDataOut()` time. Remapping output sqlvar to fbcpp::outMessage desynchronizes them when static cursors call `initStaticCursor()` (which remaps sqlvar to internal row blocks). Output `Sqlda::buffer` remains the stable target for all fetch paths. Fixing requires making the convert path read through `headSqlVarPtr` indirection instead of cached `dataPtr`. | Medium | ⏸️ Deferred |
| **14.4.7.3** | **Migrate N-row prefetch to `fbcpp::RowSet`** — Deferred. Requires `fbcpp::Statement` to own the cursor (`resultSetHandle`), but we open cursors via raw API in `execute()`. `fbcpp::Statement::execute()` auto-fetches the first row (incompatible with ODBC separate execute/fetch model). Current 64-row prefetch is functionally equivalent to RowSet. | Medium | ⏸️ Deferred |
...
| **14.4.7.5** | **Delete Sqlda class** — Blocked. `Sqlda` still provides: sqlvar property storage, static cursor support (CDataStaticCursor), metadata access helpers, `checkAndRebuild()`. Deletion requires replacing these with lighter-weight alternatives. | Easy | ❌ Blocked |
